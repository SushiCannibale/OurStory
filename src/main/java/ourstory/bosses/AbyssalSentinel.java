...
//	public Thread skills = new Thread(this);
	...
	public AbyssalSentinel(Difficulty difficulty, Monster entity) {
		super("Abyssal Sentinel", entity);
...
//	}
	...
	protected Map<Predicate<AbstractBoss>, AbstractSkill> getSkills() {
		...
		skills.put(boss -> {
			/* Triggers BroadcastMessageSkill is boss has 50% or less hearts and isn't busy */
			double max_health = boss.getEntity().getAttribute(Attribute.GENERIC_MAX_HEALTH).getValue();
			return boss.getEntity().getHealth() <= max_health * 0.5D
					&& !boss.hasRunningSkills();
		}, new BroadcastMessageSkill());
